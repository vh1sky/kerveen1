...
public class CRC8 {
    ...
    /**
     * 计算onenet平台返回数据CRC8校验值是否一致
     *
     * @param hexString   onenet平台返回数据
     * @return true or false
     */
    public static boolean calcCrc8(String hexString) {
        hexString = hexString.toUpperCase();
        byte[] dataByte = HexUtils.hexStringToByteArray(hexString.substring(0, hexString.length() - 2));
        byte[] crcByte = HexUtils.hexStringToByteArray(hexString.substring(hexString.length() - 2));
        if (CRC8.calcCrc8(dataByte) == crcByte[0]) {
            return true;
        } else {
            return false;
        }
