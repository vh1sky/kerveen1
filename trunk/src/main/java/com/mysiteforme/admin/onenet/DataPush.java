 ...
 */
@Controller
@RequestMapping("admin/onenet/dataPush")
@EnableAutoConfiguration
public class DataPush {

    private static String token ="abcdefghijkmlnopqrstuvwxyz";//用户自定义token和OneNet第三方平台配置里的token一致
    private static String aeskey ="z7EiIjdCjDvDaT1U3C/rUenCTuvxyR+lTOna+afjnL0=";//aeskey和OneNet第三方平台配置里的token一致

    private static Logger logger = LoggerFactory.getLogger(DataPush.class);
    /**
     * 功能描述：第三方平台数据接收。<p>
     *           <ul>注:
    ...
    @ResponseBody
    public String receive(@RequestBody String body) throws NoSuchPaddingException, InvalidKeyException, NoSuchAlgorithmException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException, JSONException {
        ...
        if (obj != null){
            ...
            if (dataRight){
                OnenetConstant.CONTENT.put(DateUtils.getDate(), obj.getMsg().toString());
                DataAnalyseRunner t = new DataAnalyseRunner();
